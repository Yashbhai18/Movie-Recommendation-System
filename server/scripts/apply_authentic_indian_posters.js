...
// Verified genuine Indian posters (Wikimedia & TMDB official Indian posters)
...
for (const m of movies) {
  if (verifiedPosters[m.title]) {
    m.poster_path = verifiedPosters[m.title];
    m.backdrop_path = verifiedPosters[m.title];
    count++;
  }
}

fs.writeFileSync(jsonPath, JSON.stringify(movies, null, 2), 'utf-8');
console.log(`Successfully updated ${count} Indian movie posters with 100% authentic posters!`);
